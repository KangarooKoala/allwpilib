...
static std::string FormatBytesSize(uintmax_t value) {
  ...
  if (value >= GI_B) {
    return fmt::format("{:.1f} GiB", static_cast<double>(value) / GI_B);
  } else if (value >= MI_B) {
    return fmt::format("{:.1f} MiB", static_cast<double>(value) / MI_B);
  } else if (value >= KI_B) {
    return fmt::format("{:.1f} KiB", static_cast<double>(value) / KI_B);
  } else {
