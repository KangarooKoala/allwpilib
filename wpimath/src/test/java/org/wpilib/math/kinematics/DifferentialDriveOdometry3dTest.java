...
class DifferentialDriveOdometry3dTest {
  ...
  void testInitialize() {
    ...
    assertAll(
        () -> assertEquals(pose.getX(), 1.0, EPSILON),
        () -> assertEquals(pose.getY(), 2.0, EPSILON),
        () -> assertEquals(pose.getZ(), 0.0, EPSILON),
        () -> assertEquals(pose.getRotation().toRotation2d().getDegrees(), 45.0, EPSILON));
  }
  ...
  void testOdometryWithEncoderDistances() {
    ...
    assertAll(
        ...
        () -> assertEquals(pose.getZ(), 0.0, EPSILON),
        () -> assertEquals(pose.getRotation().toRotation2d().getDegrees(), 90.0, EPSILON));
  }
}
