...
TEST(SwerveModuleStateTest, Optimize) {
  ...
  EXPECT_NEAR(refA.speed.value(), 2.0, EPSILON);
  EXPECT_NEAR(refA.angle.Degrees().value(), 0.0, EPSILON);

  wpi::math::Rotation2d angleB{-50_deg};
  wpi::math::SwerveModuleState refB{4.7_mps, 41_deg};
  refB.Optimize(angleB);

  EXPECT_NEAR(refB.speed.value(), -4.7, EPSILON);
  EXPECT_NEAR(refB.angle.Degrees().value(), -139.0, EPSILON);
}
...
TEST(SwerveModuleStateTest, NoOptimize) {
  ...
  EXPECT_NEAR(refA.speed.value(), 2.0, EPSILON);
  EXPECT_NEAR(refA.angle.Degrees().value(), 89.0, EPSILON);

  wpi::math::Rotation2d angleB{0_deg};
  wpi::math::SwerveModuleState refB{-2_mps, -2_deg};
  refB.Optimize(angleB);

  EXPECT_NEAR(refB.speed.value(), -2.0, EPSILON);
  EXPECT_NEAR(refB.angle.Degrees().value(), -2.0, EPSILON);
}
...
TEST(SwerveModuleStateTest, CosineScaling) {
  ...
  EXPECT_NEAR(refA.speed.value(), std::sqrt(2.0), EPSILON);
  EXPECT_NEAR(refA.angle.Degrees().value(), 45.0, EPSILON);
