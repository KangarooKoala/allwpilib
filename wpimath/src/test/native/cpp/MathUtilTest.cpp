...
TEST(MathUtilTest, CopySignPowWithUnits) {
  EXPECT_DOUBLE_EQ(
      0, mp::value(frc::CopySignPow<mp::quantity<mp::m / mp::s>>(0.0 * mp::m / mp::s, 2.0)));
  EXPECT_DOUBLE_EQ(
      1, mp::value(frc::CopySignPow<mp::quantity<mp::m / mp::s>>(1.0 * mp::m / mp::s, 2.0)));
  EXPECT_DOUBLE_EQ(
      -1, mp::value(frc::CopySignPow<mp::quantity<mp::m / mp::s>>(-1.0 * mp::m / mp::s, 2.0)));

  EXPECT_DOUBLE_EQ(
      0.5 * 0.5 * 10,
      mp::value(frc::CopySignPow<mp::quantity<mp::m / mp::s>>(5.0 * mp::m / mp::s, 2.0, 10.0 * mp::m / mp::s)));
  EXPECT_DOUBLE_EQ(
      -0.5 * 0.5 * 10,
      mp::value(frc::CopySignPow<mp::quantity<mp::m / mp::s>>(-5.0 * mp::m / mp::s, 2.0, 10.0 * mp::m / mp::s)
          ));
