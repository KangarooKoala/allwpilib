...
TEST(DifferentialDriveAccelerationLimiterTest, LowLimits) {
  ...
  // Ensure voltage exceeds acceleration before limiting
  {
    ...
    EXPECT_GT(wpi::units::abs(a), maxA);
  }
  {
    Vectord<2> accels =
        plant.A() * xAccelLimiter + plant.B() * Vectord<2>{-12.0, 12.0};
    wpi::units::radians_per_second_squared<> alpha{(accels(1) - accels(0)) /
                                                   trackwidth.value()};
    EXPECT_GT(wpi::units::abs(alpha), maxAlpha);
  }
  ...
  for (auto t = 0_s; t < 3_s; t += dt) {
    ...
    wpi::units::radians_per_second_squared<> alpha{(accels(1) - accels(0)) /
                                                   trackwidth.value()};
    EXPECT_LE(wpi::units::abs(a), maxA);
    EXPECT_LE(wpi::units::abs(alpha), maxAlpha);
  }
...
TEST(DifferentialDriveAccelerationLimiterTest, SeparateMinMaxLowLimits) {
  ...
  // Ensure voltage exceeds acceleration before limiting
  {
    ...
    wpi::units::meters_per_second_squared<> a{(accels(0) + accels(1)) / 2.0};
    EXPECT_GT(wpi::units::abs(a), maxA);
    EXPECT_GT(wpi::units::abs(a), -minA);
  }
