...
TEST(Translation3dTest, Sum) {
  ...
  EXPECT_NEAR(3.0, sum.X().value(), EPSILON);
  EXPECT_NEAR(8.0, sum.Y().value(), EPSILON);
  EXPECT_NEAR(13.0, sum.Z().value(), EPSILON);
}
...
TEST(Translation3dTest, Difference) {
  ...
  EXPECT_NEAR(difference.X().value(), -1.0, EPSILON);
  EXPECT_NEAR(difference.Y().value(), -2.0, EPSILON);
  EXPECT_NEAR(difference.Z().value(), -3.0, EPSILON);
}
...
TEST(Translation3dTest, RotateBy) {
  ...
  EXPECT_NEAR(rotated1.X().value(), 1.0, EPSILON);
  EXPECT_NEAR(rotated1.Y().value(), -3.0, EPSILON);
  EXPECT_NEAR(rotated1.Z().value(), 2.0, EPSILON);

  const auto rotated2 = translation.RotateBy(Rotation3d{yAxis, 90_deg});
  EXPECT_NEAR(rotated2.X().value(), 3.0, EPSILON);
  EXPECT_NEAR(rotated2.Y().value(), 2.0, EPSILON);
  EXPECT_NEAR(rotated2.Z().value(), -1.0, EPSILON);

  const auto rotated3 = translation.RotateBy(Rotation3d{zAxis, 90_deg});
  EXPECT_NEAR(rotated3.X().value(), -2.0, EPSILON);
  EXPECT_NEAR(rotated3.Y().value(), 1.0, EPSILON);
  EXPECT_NEAR(rotated3.Z().value(), 3.0, EPSILON);
}
...
TEST(Translation3dTest, RotateAround) {
  ...
  EXPECT_NEAR(rotated1.X().value(), 1.0, EPSILON);
  EXPECT_NEAR(rotated1.Y().value(), 0.0, EPSILON);
  EXPECT_NEAR(rotated1.Z().value(), 1.0, EPSILON);

  const auto rotated2 =
      translation.RotateAround(around, Rotation3d{yAxis, 90_deg});
  EXPECT_NEAR(rotated2.X().value(), 5.0, EPSILON);
  EXPECT_NEAR(rotated2.Y().value(), 2.0, EPSILON);
  EXPECT_NEAR(rotated2.Z().value(), 3.0, EPSILON);

  const auto rotated3 =
      translation.RotateAround(around, Rotation3d{zAxis, 90_deg});
  EXPECT_NEAR(rotated3.X().value(), 3.0, EPSILON);
  EXPECT_NEAR(rotated3.Y().value(), 0.0, EPSILON);
  EXPECT_NEAR(rotated3.Z().value(), 3.0, EPSILON);
}
...
TEST(Translation3dTest, Multiplication) {
  ...
  EXPECT_NEAR(mult.X().value(), 9.0, EPSILON);
  EXPECT_NEAR(mult.Y().value(), 15.0, EPSILON);
  EXPECT_NEAR(mult.Z().value(), 21.0, EPSILON);
}
...
TEST(Translation3dTest, Division) {
  ...
  EXPECT_NEAR(div.X().value(), 1.5, EPSILON);
  EXPECT_NEAR(div.Y().value(), 2.5, EPSILON);
  EXPECT_NEAR(div.Z().value(), 3.5, EPSILON);
}

TEST(Translation3dTest, Norm) {
  const Translation3d one{3_m, 5_m, 7_m};
  EXPECT_NEAR(one.Norm().value(), std::hypot(3, 5, 7), EPSILON);
}

TEST(Translation3dTest, SquaredNorm) {
  const Translation3d one{3_m, 5_m, 7_m};
  EXPECT_NEAR(one.SquaredNorm().value(), 83.0, EPSILON);
}

TEST(Translation3dTest, Distance) {
  const Translation3d one{1_m, 1_m, 1_m};
  const Translation3d two{6_m, 6_m, 6_m};
  EXPECT_NEAR(one.Distance(two).value(), 5 * std::sqrt(3), EPSILON);
}

TEST(Translation3dTest, SquaredDistance) {
  const Translation3d one{1_m, 1_m, 1_m};
  const Translation3d two{6_m, 6_m, 6_m};
  EXPECT_NEAR(one.SquaredDistance(two).value(), 75.0, EPSILON);
}
...
TEST(Translation3dTest, UnaryMinus) {
  ...
  EXPECT_NEAR(inverted.X().value(), 4.5, EPSILON);
  EXPECT_NEAR(inverted.Y().value(), -7, EPSILON);
  EXPECT_NEAR(inverted.Z().value(), -9, EPSILON);
}
...
TEST(Translation3dTest, PolarConstructor) {
  ...
  EXPECT_NEAR(one.X().value(), 1.0, EPSILON);
  EXPECT_NEAR(one.Y().value(), 1.0, EPSILON);
  EXPECT_NEAR(one.Z().value(), 0.0, EPSILON);

  Translation3d two{2_m, Rotation3d{zAxis, 60_deg}};
  EXPECT_NEAR(two.X().value(), 1.0, EPSILON);
  EXPECT_NEAR(two.Y().value(), std::sqrt(3.0), EPSILON);
  EXPECT_NEAR(two.Z().value(), 0.0, EPSILON);
}
...
TEST(Translation3dTest, Dot) {
  ...
  EXPECT_NEAR(one.Dot(two).value(), 32.0, EPSILON);
}
...
TEST(Translation3dTest, Cross) {
  ...
  EXPECT_NEAR(cross[0].value(), -3.0, EPSILON);
  EXPECT_NEAR(cross[1].value(), 6.0, EPSILON);
  EXPECT_NEAR(cross[2].value(), -3.0, EPSILON);
}
