...
TEST(TrapezoidProfileTest, PosContinuousUnderVelChange) {
  ...
  auto state = profile.Calculate(
      DT, wpi::math::TrapezoidProfile<wpi::units::meter>::State{}, goal);
  ...
  for (int i = 0; i < 1600; ++i) {
    ...
    state = profile.Calculate(DT, state, goal);
    auto estimatedVel = (state.position - lastPos) / DT;
...
TEST(TrapezoidProfileTest, TimingToGoal) {
  ...
  wpi::math::TrapezoidProfile<wpi::units::meter> profile{constraints};
  auto state = profile.Calculate(
      DT, goal, wpi::math::TrapezoidProfile<wpi::units::meter>::State{});
