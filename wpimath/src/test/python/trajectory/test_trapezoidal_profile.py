...
def test_pos_continuous_under_vel_change():
    ...
    profile = TrapezoidProfile(constraints)
    state = profile.calculate(DT, TrapezoidProfile.State(), goal)
    ...
    for i in range(1600):
        ...
        state = profile.calculate(DT, state, goal)
        estimated_vel = (state.position - last_pos) / DT
...
def test_timing_to_goal():
    ...
    state = profile.calculate(DT, goal, TrapezoidProfile.State())
...
def test_timing_before_goal():
    ...
    for i in range(400):
        ...
        if not reached_goal and abs(state.velocity - 1.0) < 1e-4:
            assert math.isclose(predicted_time_left, i * DT, abs_tol=0.02)
            reached_goal = True
