...
class WPILIB_DLLEXPORT Translation2d {
 public:
   ...
   */
  constexpr Translation2d(wpi::units::meters<> x, wpi::units::meters<> y)
      : m_x{x}, m_y{y} {}
   ...
   */
  constexpr wpi::units::meters<> Distance(const Translation2d& other) const {
    return wpi::units::hypot(other.m_x - m_x, other.m_y - m_y);
  }
  ...
  constexpr wpi::units::meters<> Norm() const {
    return wpi::units::hypot(m_x, m_y);
  }
  ...
  constexpr wpi::units::square_meters<> SquaredNorm() const {
    return wpi::units::pow<2>(m_x) + wpi::units::pow<2>(m_y);
  }
   ...
   */
  constexpr wpi::units::square_meters<> Dot(const Translation2d& other) const {
