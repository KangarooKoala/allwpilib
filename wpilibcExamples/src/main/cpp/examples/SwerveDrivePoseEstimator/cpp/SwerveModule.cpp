...
wpi::math::SwerveModuleState SwerveModule::GetState() const {
  return {wpi::units::meters_per_second<>{m_driveEncoder.GetRate()},
          wpi::units::radians<>{m_turningEncoder.GetDistance()}};
}

wpi::math::SwerveModulePosition SwerveModule::GetPosition() const {
  return {wpi::units::meters<>{m_driveEncoder.GetDistance()},
          wpi::units::radians<>{m_turningEncoder.GetDistance()}};
}
...
void SwerveModule::SetDesiredState(
  ...
  m_driveMotor.SetVoltage(wpi::units::volts<>{driveOutput} + driveFeedforward);
  m_turningMotor.SetVoltage(wpi::units::volts<>{turnOutput} + turnFeedforward);
}
